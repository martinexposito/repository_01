...
import {
  ...
  getPlugsurfingDiagnostics,
  getTomTomDiagnostics
} from './availability-provider.js';
...
const server = createServer(async (req, res) => {
  try {
    ...
    if (req.method === 'GET' && url.pathname === '/debug/providers') {
      return sendJson(res, 200, getProviderDiagnostics());
    }

    if (req.method === 'GET' && url.pathname === '/debug/tomtom') {
      return sendJson(res, 200, await getTomTomDiagnostics({
        lat: Number(url.searchParams.get('lat')) || undefined,
        lng: Number(url.searchParams.get('lng')) || undefined,
        radiusKm: Number(url.searchParams.get('radiusKm')) || undefined
      }));
    }

    if (req.method === 'GET' && url.pathname === '/debug/here') {
      return sendJson(res, 200, await getHereDiagnostics({
        lat: Number(url.searchParams.get('lat')) || undefined,
        lng: Number(url.searchParams.get('lng')) || undefined,
        radiusKm: Number(url.searchParams.get('radiusKm')) || undefined
      }));
    ...
    if (req.method === 'GET' && url.pathname === '/debug/plugsurfing') {
      return sendJson(res, 200, await getPlugsurfingDiagnostics({
        lat: Number(url.searchParams.get('lat')) || undefined,
        lng: Number(url.searchParams.get('lng')) || undefined,
        radiusKm: Number(url.searchParams.get('radiusKm')) || undefined
      }));
