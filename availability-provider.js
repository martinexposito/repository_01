...
const DEFAULT_HERE_SEARCH_BASE_URL = 'https://discover.search.hereapi.com/v1';
...
export async function getHereDiagnostics({ lat = 52.5308, lng = 13.3849, radiusKm = 0.5 } = {}) {
  ...
  try {
    ...
    const detailResults = response.ok
      ? await Promise.allSettled(items.slice(0, 3).map(item => fetchHereLocationDetail(item.id)))
      : [];
    ...
    return {
      ...
      detail: {
        ...
        sample: details.slice(0, 3).map(summarizeHereStation),
        errors: detailResults
...
async function getHereAvailability({ center, radiusKm, stations }) {
  ...
  try {
    ...
    const items = getHereStations(data);
    const details = await Promise.allSettled(items.slice(0, 50).map(item => fetchHereLocationDetail(item.id)));
    return details
      .filter(result => result.status === 'fulfilled' && result.value)
      .map(result => normalizeHereStation(result.value, stations))
      .filter(Boolean);
  } catch (error) {
...
function buildHereLocationDetailUrl(id) {
  const url = new URL(`${getHereEvBaseUrl()}/locations/${encodeURIComponent(id)}`);
  url.searchParams.set('apiKey', process.env.HERE_API_KEY);
  return url;
}

async function fetchHereLocationDetail(id) {
  if (!id) return null;
  const response = await fetch(buildHereLocationDetailUrl(id), {
    headers: { Accept: 'application/json' }
  });
  const text = await response.text();
  const body = parseJson(text);
  if (!response.ok) {
    throw new Error(`HERE detail ${response.status}: ${summarizeProviderError(body, text) || response.statusText}`);
  }
  return getHereLocation(body);
}
...
function normalizeHereStation(station, stations) {
  ...
  const statusItems = evses.length ? evses : [station];
  const knownItems = statusItems.filter(item => getHereStatus(item) !== 'unknown');
  const availableConns = statusItems.filter(item => getHereStatus(item) === 'available').length;
  const totalConns = Math.max(statusItems.length, 1);
...
function getHereLocation(body) {
  ...
  if (Array.isArray(body.locations) && body.locations[0]) return body.locations[0];
  if (Array.isArray(body.items) && body.items[0]) return body.items[0];
  return body;
}
...
function getHereStatus(item) {
  const value = String(item?.status ?? item?.availabilityStatus ?? item?.availability?.status ?? '').toUpperCase();
...
function getHereConnectors(station, statusItems) {
  const connectors = [
    ...
    ...statusItems.flatMap(item => Array.isArray(item.connectors) ? item.connectors : [])
  ];
  const source = connectors.length ? connectors : statusItems;
  return source.map(item => ({
    type: item.connectorType || item.type || item.standard || 'Conector EV',
    kw: numberOrNull(item.powerKW ?? item.powerKw ?? item.maxPowerKW ?? (item.power ? Number(item.power) / 1000 : null)),
    status: getHereStatus(item)
  }));
}
...
function summarizeHereStation(station) {
  ...
  const statuses = (evses.length ? evses : [station]).reduce((acc, item) => {
    const status = item?.status || item?.availabilityStatus || item?.availability?.status || 'UNKNOWN';
