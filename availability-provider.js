...
const DEFAULT_HERE_EV_BASE_URL = 'https://ev-v3.charging.ext.here.com/v3';
...
export async function getHereDiagnostics({ lat = 52.5308, lng = 13.3849, radiusKm = 0.5 } = {}) {
  ...
  const url = buildHereStationsUrl({ lat, lng, radiusKm });
  try {
    ...
    const stations = getHereStations(body);
    ...
    return {
      ...
      response: {
        ...
        sample: stations.slice(0, 5).map(summarizeHereStation)
      }
    };
  } catch (error) {
    return {
...
async function getHereAvailability({ center, radiusKm, stations }) {
  ...
  try {
    const response = await fetch(buildHereStationsUrl({
      lat: center.lat,
      lng: center.lng,
      radiusKm
    }), {
    ...
    return getHereStations(data)
      .map(station => normalizeHereStation(station, stations))
      .filter(Boolean);
  } catch (error) {
...
function buildHereStationsUrl({ lat, lng, radiusKm }) {
  const url = new URL(`${getHereBaseUrl()}/stations`);
